...
export default async function deleteCustomerService(
  handle: string,
  token: string,
  id: string | number,
) {
  const url = `https://${handle}.myshopline.com/admin/openapi/${API_VERSION}/customers/${id}.json`;
  console.log('[deleteCustomerService] DELETE', url);
  ...
  console.log('[deleteCustomerService] status:', response.status);
